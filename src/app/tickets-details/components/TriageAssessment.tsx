...
import {
  ...
  formatName,
  getCriticalityColor,
} from "@/app/hooks/useTriageAssessment";
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {data.required_skills.map((skill, index) => (
              <span
                key={`skill-${index}`}
                className={`px-2 py-1 text-xs font-medium rounded-md ${getCriticalityColor(
                  skill.criticality
                )} text-white`}
              >
                {skill.skill_name}
      ...
      <div className="pt-4">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {data.required_parts.map((part, index) => (
            <span
              key={`part-${index}`}
              className="text-white border border-purple-500 px-3 py-1 rounded-md font-medium hover:bg-purple-500 hover:text-white transition"
            >
