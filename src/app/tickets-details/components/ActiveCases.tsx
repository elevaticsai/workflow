...
const ActiveCases: React.FC = () => {
  ...
  const { selectedTicket, setSelectedTicket, selectFirstTicket } =
    useTicketContext();
  ...
  return (
    <div className="bg-gray-800 rounded-lg shadow-lg overflow-hidden border border-gray-700">
      <div className="p-4 bg-gray-800/80 border-b border-gray-700 flex justify-between items-center">
        ...
        {isLoading && <div className="text-sm text-blue-400">Loading...</div>}
      </div>
        ...
        <div className="divide-y divide-gray-700">
          {tickets.length === 0 && !isLoading ? (
            ...
            tickets.map((ticket, index) => {
              ...
              const originalData =
                data && Array.isArray(data) ? data[index] : null;

              // Check if this ticket is selected
              const isSelected =
                selectedTicket?.ticket_id === originalData?.ticket_id;

              return (
                <div
                  key={ticket.id}
                  onClick={() =>
                    originalData && setSelectedTicket(originalData)
                  }
