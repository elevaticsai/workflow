...
/**
 * API route handler for triage assessment data
...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `https://workforcev2.elevatics.site/api/triage/${ticketId}`,
      {
        headers: {
    ...
    console.error(
      `Error fetching triage assessment for ticket ${ticketId}:`,
      error
    );
    return NextResponse.json(
      { error: "Failed to fetch triage assessment" },
      { status: 500 }
    );
  }
}
